...
本章节定义项目中所有图片资源的格式、尺寸、命名与风格要求。后续各界面设计文档将基于此规范引用具体素材。

### 11.1 图片格式
| 格式 | 适用场景 | 说明 |
|------|----------|------|
| **PNG** | 需要透明通道的 UI 元素（图标、印章、人物立绘） | 优先使用 PNG-24，保证边缘平滑 |
| **JPG / WebP** | 不需要透明的背景图、场景图 | WebP 优先，兼顾画质与体积；JPG 作为降级方案 |
| **SVG** | 图标、简单矢量装饰 | 优先使用 SVG，便于缩放与变色 |

### 11.2 图片尺寸规范

| 类型 | 推荐尺寸（宽×高） | 说明 |
|------|-------------------|------|
| **全屏背景图** | 1920×1080px（桌面端），750×1334px（移动端） | 覆盖主流分辨率，居中裁剪或自适应缩放 |
| **人物立绘（半身像）** | 512×512px 或 600×800px | 透明背景 PNG，角色居中，底部留出 10% 空间避免被 UI 遮挡 |
| **人物头像（缩略图）** | 128×128px 或 256×256px | 用于列表、图鉴网格，透明背景 PNG |
| **场景插图（城镇/室内）** | 1920×1080px | 全屏沉浸式背景，JPG/WebP，可带轻微景深效果 |
| **UI 图标（线性）** | 24×24px 或 32×32px | SVG 优先，PNG 备选，描边 2px |
| **功能入口图标** | 64×64px 或 80×80px | PNG 或 SVG，用于城镇据点中的设施卡片 |
| **印章装饰** | 60×60px 至 100×100px | PNG 透明背景，白字朱红底 |
| **卷轴/边框装饰** | 宽度 400px-600px，高度按比例 | PNG 透明背景，九宫格拉伸或整体使用 |

### 11.3 图片风格指南

| 类型 | 风格要求 | 参考关键词 |
|------|----------|------------|
| **背景图（宣纸/纹理）** | 微黄宣纸底，可带极淡的纤维颗粒感或水墨晕染痕迹 | 仿古宣纸、手工纸纹理、淡墨渲染 |
| **背景图（场景）** | 水墨画风或工笔淡彩，色彩饱和度低，避免艳丽 | 水墨江山、明代街景、竹林远山、皇城远眺 |
| **人物立绘** | 工笔重彩或水墨写意风格，服饰符合明代特征，表情端庄 | 明代官员、仕女图、历史人物肖像 |
| **图标** | 线性勾勒，融入云纹、回纹、如意纹等传统纹样，避免过度复杂 | 古风线性图标、明朝纹样、印章风格 |
| **装饰元素** | 印章、栏线、云纹、卷轴边缘，不可喧宾夺主 | 朱砂印章、古籍版刻栏线、水墨笔触 |

### 11.4 图片命名规范

遵循 `[类型]_[描述]_[尺寸].扩展名` 格式，示例：

| 文件名 | 说明 |
|--------|------|
| `bg_ink_mountain_1920x1080.webp` | 水墨江山背景图 |
| `char_zhu_yuanzhang_512x512.png` | 朱元璋半身立绘 |
| `icon_sword_24x24.svg` | 刀剑图标 |
| `stamp_imperial_80x80.png` | “敕令”印章 |
| `scene_yingtian_street_1920x1080.jpg` | 应天府街景 |

### 11.5 图片在 CSS 中的引用方式

为便于主题切换与响应式适配，图片路径应定义为 CSS 变量：

```css
:root {
  --bg-main: url('../assets/images/backgrounds/bg_paper_texture_1920x1080.webp');
  --bg-dark-overlay: url('../assets/images/backgrounds/bg_ink_wash_dark_1920x1080.webp');
  --decor-scroll-border: url('../assets/images/ui/decor_scroll_border_vertical.png');
}