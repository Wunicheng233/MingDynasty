...
window.EconomicCalculator = {
    ...
    /**
     * 根据身份计算每月俸禄
     * @param {string} roleId - 身份ID
     * @returns {number} 每月俸禄（文）
     */
    calculateMonthlySalary(roleId) {
        // 俸禄表（单位：贯/月）
        const salaryTable = {
            'peasant': 0,
            'monk': 0,
            ...
            'thousand': 40,    // 千户
            'commandant': 80,  // 指挥佥事
            'commandant_junior': 120,  // 指挥同知
            'commandant_senior': 180,  // 指挥使
            'duke_junior': 240,  // 都督佥事
            'duke': 280,        // 都督同知
            'duke_senior': 300  // 五军都督
        };
        const monthlyGuan = salaryTable[roleId] || 0;
        return this.guanToWen(monthlyGuan);
    },

    /**
     * 计算两个月俸禄（评定会发放）
     * @param {string} roleId - 身份ID
     * @returns {number} 两个月俸禄（文）
     */
    calculateBiMonthlySalary(roleId) {
        return this.calculateMonthlySalary(roleId) * 2;
    }
};
